...
log_file = f'./logs/training_ver1_{args.model}_{args.dataset}_info.log' # 日志文件名称
...
best_accuracy = 0.0
current_step = 0
ballData_path = './gb_data/{}_{}_ballData_ver1.npy'.format(args.dataset,args.model)
...
for epoch in range(1,1+args.epoch):
    ...
    for batch_index,(texts,label) in enumerate(train_iterator):
        ...
        if epoch >=5:
            ### (QQQ)粒球维护栈是从一开始就维护还是从第5个epoch开始维护
            ...
            if step % 100 == 0: #每100次做一次评估         
                ...
                with torch.no_grad():
                    ...
                    if vote_test_accuracy > best_accuracy:
                        ...
                        file_path = './gb_data/{}_{}_ballData_ver1.npy'.format(args.dataset,args.model) # 保存的npy文件路径 
