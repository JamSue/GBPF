...
def calculate_center_and_radius(gb):
    ...
    # 余弦计算半径
    cosine_distances = []
    for point in data_no_label:
        cosine_similarity = numpy.dot(point, center) / (numpy.linalg.norm(point) * numpy.linalg.norm(center))
        cosine_distances.append(1 - cosine_similarity)  # 转换成余弦距离

    # 计算平均余弦距离作为半径
    radius = numpy.mean(cosine_distances)
...
def gb_plot(gb_dict, plt_type=0):
    color = {0: 'r', 1: 'k', 2: 'b', 3: 'y', 4: 'g', 5: 'c', 6: 'm', 7: 'peru', 8: '=4ink', 9: 'gold'}
...
def splits(purity, gb_dict):
    """
    gb_dict 是所有的粒球构成的一个字典，一个键值对一个粒球
    """
    gb_len = 1
    ...
    while True:
        ...
        for i in range(0, gb_len):  
            #取一个粒球放在gb_single字典中 
            gb_single = {}
            gb_dict_temp = gb_dict_single.popitem()  # 取字典数据，包括键值 随机返回的一个键值对(为元组)并从原来字典中删除
            ...
            # 取出value:粒球数据
            gb = gb_dict_temp[1][0] # gb_dict_temp是一个包含键值对的元组，从元组中取出
            # print("粒球数据\n", gb)  # 当前球内样本向量[球个数，65]
            len_label = numpy.unique(gb[:, 0], axis=0) # 求粒球中所有行的第一列，并取第一列的唯一值的个数。即为标签的个数
            cnt = gb.shape[0]
            # 判断纯度是否满足要求，不满足则继续划分
            label, p = get_label_and_purity(gb)
            if p < purity:# or (cnt>=30 and len_label.shape[0]>=2):  # 当前纯度小于设定纯度时，停止划分，更新字典，进行下一步
                # print("gb_single\n", gb_single)
...
def calculate_distances(data, p):
    """
    dis_all = ((data-p)**2).sum()**0.5
    data = data.reshape(8,-1)
    p = p.reshape(8,-1)
    dis = ((data-p)**2).sum(axis=1)
    dis_top3 = (np.sort(dis)[-3:])**0.5
    return 0.7*dis_top3.sum() + 0.3*dis_all
    """
    #### 余弦距离
    cos_similarities = np.dot(data, p) / (np.linalg.norm(data) * np.linalg.norm(p))
    # 将相似度转换为距离（距离 = 1 - 相似度）
    dis = 1 - cos_similarities
    return dis
...
def main(data, pur):  #+pur
    ...
    # data [batch_size,label+features]
    times = 0
    num_gb = 0
    for i in range(0, 6):
        start = time.time()
        ...
        for data_single in data_list:
            if data_single[1:] not in data_temp:
                data_temp.append(data_single[1:])#  data_temp:数据的特征维度
                data.append(data_single) # data: [label1+features1,....]
        data = np.array(data)
        # print("single_list\n", data)   # data no change
        ...
        # 封装成字典 =》相当于一个球 字典的键是中心向量构成的字符串，值是所以数据以及每条数据离中心的距离
        gb_dict = {}
        gb_dict_key = str(center_init.tolist()[0]) #初始中心的label
        # print(gb_dict_key) 
        # print(len(center_init)) # 301 (label+300features)
...
if __name__ == '__main__':
    # 随机生成数据，数据前五个标签为0后面的标签为1
    data = np.random.normal(loc=0,scale=1,size=(10,48))
    data[:5,0] = 0
    data[5:,0] = 1
   # print(data)
    numbers, result, centers, radius = main(data,0.6)

    for id,n in enumerate(numbers):
        print("第{}个中总数{}".format(id, n))
        print("球心：",centers[id])
        print("半径：",radius[id])
        print("球内样本：")
        for i in result[id]:
