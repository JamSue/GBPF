...
log_file = f'./logs/ablation_metricLearning_{args.model}_{args.dataset}_info.log'  # 日志文件名称
...
for epoch in range(1, 1 + args.epoch):
    ...
    if epoch < 3:
       ...
       #     print("1label",label.shape)
            ...
            features["input_ids"] = (features["input_ids"].view(args.batch_size, maxlen)).to(
                device)  # (batch_size,seq_len)
            features["attention_mask"] = (features["attention_mask"].view(args.batch_size, maxlen)).to(device)
            if args.model == 'RoBERTa':
                features["token_type_ids"] = None
            if args.model == 'Bert' or args.model == 'XLNet':
                features["token_type_ids"] = (features["token_type_ids"].view(args.batch_size, maxlen)).to(
                    device)
            ...
            entropyLoss = cross_entropy_loss(output,center_label.long()) # 第一个参数传logit 第二个参数传目标标签
            
            optimizer.zero_grad()  # 梯度清零
            entropyLoss.backward()  # 反向传播 通过模型的反向传播函数进行传播的
