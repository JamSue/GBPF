    ...
    if epoch < 3:
       ...
       #     print("1label",label.shape)
            ...
            negtiveFeature = Tokenizer(negtiveSample, max_length=maxlen, padding='max_length', truncation=True,
                                       return_tensors='pt')
            augmentFeature = Tokenizer(augmentSample, max_length=maxlen, padding='max_length', truncation=True,
                                       return_tensors='pt')

            del positiveSample, negtiveSample, augmentSample
            ...
            # positiveFeature["input_ids"] = (positiveFeature["input_ids"].view(args.batch_size, maxlen)).to(device)  # (batch_size,seq_len)
            # positiveFeature["attention_mask"] =(positiveFeature["attention_mask"].view(args.batch_size, maxlen)).to(device)
            # positiveFeature["token_type_ids"] = None
            # if args.model=='Bert'or args.model=='XLNet':
            #     positiveFeature["token_type_ids"] = (positiveFeature["token_type_ids"].view(args.batch_size, maxlen)).to(device)

            # negtiveFeature["input_ids"] = (negtiveFeature["input_ids"].view(args.batch_size, maxlen)).to(device)  # (batch_size,seq_len)
            # negtiveFeature["attention_mask"] =(negtiveFeature["attention_mask"].view(args.batch_size, maxlen)).to(device)
            # if args.model=='Bert'or args.model=='XLNet':
            #     negtiveFeature["token_type_ids"] = (negtiveFeature["token_type_ids"].view(args.batch_size, maxlen)).to(device)

            # augmentFeature["input_ids"] = (augmentFeature["input_ids"].view(args.batch_size, maxlen)).to(device)  # (batch_size,seq_len)
            # augmentFeature["attention_mask"] =(augmentFeature["attention_mask"].view(args.batch_size, maxlen)).to(device)
            # if args.model=='Bert'or args.model=='XLNet':
            #     augmentFeature["token_type_ids"] = (augmentFeature["token_type_ids"].view(args.batch_size, maxlen)).to(device)
            ...
            augmentLogits = model(augmentFeature["input_ids"],augmentFeature["attention_mask"],augmentFeature["token_type_ids"], label, flag=2, purity=args.purity)
            del augmentFeature
            ...
            dis_aug_pos = torch.zeros(data.shape[0], device=device)
            dis_aug_neg = torch.zeros(data.shape[0], device=device)
