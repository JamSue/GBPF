'样本分别与正样本/负样本、正粒球中心/负粒球中心 进行度量学习'
import os
import logging
import numpy as np
import torch
...
log_file = f'./logs/training222_{args.model}_{args.dataset}_info.log' # 日志文件名称
...
best_accuracy = 0.0
current_step = 0
ballData_path = './gb_data/{}_{}_ballData_222.npy'.format(args.dataset,args.model)
...
for epoch in range(1,1+args.epoch):
    ...
    if epoch < 5:
        ...
        for batch_index,(texts,label) in enumerate(train_iterator):
            ...
            loss = entropyLoss + args.a*(dis+dis_center_pos) - args.b*(neg_dis+dis_center_neg)
    ...
    if epoch >=5:
        ...
        for batch_index,(texts,label) in enumerate(train_iterator):
            ...
            # loss =  Loss(output,center_label.long(),dis_center_pos,dis_center_neg).to(args.cpu_device)
            ...
            if step % 100 == 0: #每100次做一次评估         
                ...
                with torch.no_grad():
                    ...
                    if vote_test_accuracy > best_accuracy:
                        ...
                        file_path = './gb_data/{}_{}_ballData_222.npy'.format(args.dataset,args.model) # 保存的npy文件路径 
